  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', color: '#F8FAFC' }}>
      ...
      <div style={{ marginBottom: '3rem', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '2.8rem', fontWeight: 900, color: '#FFFFFF' }}>التحكم في سعر الصرف</h1>
        <p style={{ color: '#94A3B8', fontSize: '1rem', marginTop: '0.5rem' }}>إدارة قيمة العملات الأجنبية (USD/AED) مقابل الجنيه المصري</p>
      </div>
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={cardStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2.5rem' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.25rem', fontWeight: 900, color: '#FFFFFF' }}>قيمة الدولار / الدرهم</h2>
              <p style={{ fontSize: '0.85rem', color: '#64748B' }}>سيتم استخدام هذه القيمة لحساب كافة أسعار الجملة تلقائياً</p>
            </div>
          </div>

          <div style={{ marginBottom: '2.5rem' }}>
            <label style={{ fontSize: '0.9rem', fontWeight: 800, color: '#94A3B8', marginBottom: '0.75rem', display: 'block' }}>سعر الصرف الحالي (EGP)</label>
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <input 
                type="number" step="0.01" value={rate} onChange={e => setRate(Number(e.target.value))}
                style={{ width: '100%', padding: '1.25rem 1.5rem', borderRadius: 16, background: 'rgba(0,0,0,0.3)', color: '#fff', border: '2px solid rgba(6, 182, 212, 0.2)', fontSize: '2rem', fontWeight: 900, outline: 'none', textAlign: 'center' }}
              />
          ...
          <button 
            ...
            style={{ width: '100%', padding: '1.25rem', background: '#06B6D4', color: '#fff', border: 'none', borderRadius: 18, fontWeight: 900, fontSize: '1.2rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', boxShadow: '0 8px 32px rgba(6,182,212,0.3)', transition: 'all 0.2s' }}
          >
      ...
      <div style={{ marginTop: '4rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        <div style={{ ...cardStyle, maxWidth: 'none', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div><p style={{ fontSize: '0.75rem', color: '#64748B' }}>آخر مزامنة ناجحة</p><p style={{ fontWeight: 800 }}>اليوم، {new Date().toLocaleTimeString('ar-EG')}</p></div>
        </div>
        <div style={{ ...cardStyle, maxWidth: 'none', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ padding: '0.75rem', background: 'rgba(6,182,212,0.1)', borderRadius: 12 }}><RefreshCw size={24} color="#06B6D4" /></div>
          <div><p style={{ fontSize: '0.75rem', color: '#64748B' }}>دقة الحساب</p><p style={{ fontWeight: 800 }}>أوتوماتيكي بالكامل (Real-time)</p></div>
        </div>
      </div>
