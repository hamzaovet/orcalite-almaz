...
export default function EmployeesPage() {
  ...
  const inpProps = { style: {
    width: '100%', padding: '0.8rem 1rem', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: '#0a0a0a', color: '#fff', outline: 'none', fontFamily: 'inherit', marginTop: '0.35rem'
  }}
  const lblStyle = { fontSize: '0.8rem', fontWeight: 600, color: 'rgba(255,255,255,0.6)' }
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          ...
          <h1 style={{ fontSize: '1.8rem', fontWeight: 900, color: '#FFFFFF' }}>صلاحيات الموظفين</h1>
        </div>
        <button onClick={openNew} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#0ea5e9', color: '#fff', border: 'none', padding: '0.75rem 1.4rem', borderRadius: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.9rem' }}>
          <Plus size={18} />
          إضافة موظف جديد
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: '#111111', borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
          {users.length === 0 ? (
            ...
            <table style={{ width: '100%', textAlign: 'right', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
              <thead>
                <tr style={{ background: '#0a0a0a', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                  <th style={{ padding: '1rem 1.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>الاسم بالكامل</th>
                  <th style={{ padding: '1rem 1.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>إسم الدخول</th>
                  <th style={{ padding: '1rem 1.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>الصلاحية</th>
                  <th style={{ padding: '1rem 1.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>تعيين فرع</th>
                  <th style={{ padding: '1rem 1.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600, width: 120 }}>إجراءات</th>
                </tr>
              </thead>
              <tbody>
                {users.map(u => (
                  <tr key={u.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.2s', ...({ ':hover': { background: 'rgba(255,255,255,0.02)' } } as any) }}>
                    <td style={{ padding: '1.25rem 1.5rem', fontWeight: 600, color: '#fff' }}>{u.name}</td>
                    ...
                    <td style={{ padding: '1.25rem 1.5rem' }}>
                       {u.role === 'Cashier' ? (
                         <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem' }}>
                    ...
                    <td style={{ padding: '1.25rem 1.5rem' }}>
                       <div style={{ display: 'flex', gap: '0.5rem' }}>
                         <button onClick={() => openEdit(u)} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', padding: '0.5rem', borderRadius: 8, cursor: 'pointer', color: '#fff' }}><Edit2 size={16} /></button>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
          <form style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)', width: '100%', maxWidth: 480, borderRadius: 20, padding: '2rem' }} onSubmit={handleSave}>
             <h2 style={{ fontSize: '1.3rem', fontWeight: 800, color: '#fff', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
             ...
             <div style={{ display: 'flex', gap: '0.75rem', marginTop: '2rem' }}>
                <button type="button" onClick={() => setModal(false)} style={{ flex: 1, padding: '0.8rem', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', borderRadius: 12, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>إلغاء</button>
                <button type="submit" disabled={submitting} style={{ flex: 1, padding: '0.8rem', background: '#0ea5e9', border: 'none', color: '#fff', borderRadius: 12, cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: 'inherit', fontWeight: 700, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      ...
      {/* Toast Notification */}
      {toast && (
        <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', background: toast.type === 'ok' ? '#22c55e' : '#ef4444', color: '#fff', padding: '1rem 1.5rem', borderRadius: 12, fontWeight: 700, fontSize: '0.9rem', boxShadow: '0 8px 30px rgba(0,0,0,0.3)', zIndex: 9999, animation: 'float-in 0.3s ease-out forwards' }}>
          {toast.msg}
        </div>
      )}
