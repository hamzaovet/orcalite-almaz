...
export default function ClientShell({
  ...
  const renderModals = () => (
    <AnimatePresence>
      {showDemoModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)' }}>
          <motion.div initial={{ opacity: 0, y: 30, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.9 }}
            style={{ width: '100%', maxWidth: 450, background: '#FFFFFF', borderRadius: 28, padding: '2.5rem', border: '1px solid #E2E8F0', boxShadow: '0 25px 60px rgba(0,0,0,0.1)', textAlign: 'center', position: 'relative' }}
          >
            <button onClick={() => setShowDemoModal(false)} style={{ position: 'absolute', top: 16, right: 16, background: 'transparent', border: 'none', cursor: 'pointer', color: '#64748B' }}><X size={24} /></button>
            <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(239,68,68,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem', border: '2px solid rgba(239,68,68,0.1)' }}>
              <ShieldAlert size={40} color="#EF4444" />
            </div>
            <h2 style={{ fontSize: '1.6rem', fontWeight: 900, color: '#0F172A', marginBottom: '1rem', letterSpacing: '0.02em' }}>عفواً، حساب تجريبي!</h2>
