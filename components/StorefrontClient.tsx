...
export function StorefrontClient({ 
  ...
  const inp: React.CSSProperties = { width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(6,182,212,0.2)', borderRadius: 12, padding: '0.85rem 1rem', color: '#fff', outline: 'none', fontSize: '0.95rem', fontFamily: 'inherit', boxSizing: 'border-box' as const }
  ...
  return (
    <>
      {/* ── Hero Section ── */}
      <section style={{ padding: '4rem 1.5rem', background: '#0B1120', display: 'flex', justifyContent: 'center' }}>
        <div style={{ position: 'relative', width: '100%', maxWidth: 1200, height: 'clamp(480px, 65vw, 720px)', borderRadius: '2.5rem', overflow: 'hidden', boxShadow: '0 42px 100px rgba(0,0,0,0.35)' }}>
          <video src="/assets/trailer.mp4" autoPlay loop muted playsInline preload="auto" className="absolute inset-0 w-full h-full object-cover z-0 opacity-50" />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(11,17,32,0.5) 0%, rgba(11,17,32,0.85) 100%)', backdropFilter: 'blur(2px)' }} />
              ...
              <AnimatePresence>
                {(trackResults || trackError) && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 10 }}
                    style={{ position: 'absolute', top: '120%', left: 0, right: 0, background: '#0F172A', border: '1px solid rgba(6,182,212,0.3)', borderRadius: 24, padding: '1.5rem', boxShadow: '0 20px 60px rgba(0,0,0,0.6)', zIndex: 100, textAlign: 'right' }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                       <span style={{ fontSize: '0.8rem', fontWeight: 800, color: '#06B6D4' }}>نتائج البحث</span>
                       <button onClick={() => { setTrackResults(null); setTrackError(null) }} style={{ background: 'transparent', border: 'none', color: '#94A3B8', cursor: 'pointer' }}><X size={16} /></button>
                    </div>
                    ...
                    {trackError ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                        {trackResults.map((t: any) => (
                          <div key={t.idSuffix} style={{ background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: 16, border: '1px solid rgba(255,255,255,0.05)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                              <span style={{ fontWeight: 900, color: '#fff' }}>{t.deviceModel}</span>
      ...
      <section style={{ padding: '6rem 2rem', background: '#0B1120', position: 'relative' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 900, color: '#FFFFFF', marginBottom: '1rem' }}>
      ...
      {/* ── Categories Grid ── */}
      <section id="categories" style={{ padding: '6rem 2rem', background: '#070D19' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 900, color: '#FFFFFF' }}>منظومة ORCA المتكاملة</h2>
          </div>
              ...
              return (
                ...
                >
                  <Link href={`/category/${cat.slug}`} style={{ textDecoration: 'none' }}>
                    <div
                      style={{
                        position: 'relative', background: '#0B1120', border: '1px solid rgba(255,255,255,0.05)',
      ...
      <section style={{ padding: '7rem 2rem', background: 'linear-gradient(135deg, #070D19 0%, #0B1120 50%, #070D19 100%)', position: 'relative', overflow: 'hidden' }}>
      ...
      <section style={{ padding: '7rem 2rem', background: 'linear-gradient(135deg, #0B1120 0%, #070D19 50%, #0B1120 100%)', position: 'relative', overflow: 'hidden' }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 40 }}
              ...
              <div style={{ padding: '2.25rem 2.25rem 1.75rem', background: 'linear-gradient(135deg, rgba(16,185,129,0.08) 0%, transparent 100%)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <p style={{ color: '#64748B', fontSize: '0.85rem' }}>صف المشكلة وارفع صورة وسنرد بالتكلفة فوراً</p>
                </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 40 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 40 }}
              ...
              <div style={{ padding: '2.25rem 2.25rem 1.75rem', background: 'linear-gradient(135deg, rgba(168,85,247,0.08) 0%, transparent 100%)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <p style={{ color: '#64748B', fontSize: '0.85rem' }}>أدخل البيانات وسنرد بأعلى سعر خلال دقائق</p>
                </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                      ...
                      <div>
                        ...
                        <select style={{...inp, background: '#111827'}} value={evalForm.storage} onChange={e => setEvalForm(f => ({...f, storage: e.target.value}))}>
